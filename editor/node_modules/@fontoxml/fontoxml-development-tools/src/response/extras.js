...
export default {
	...
	dotSpinner: function dotSpinnerFactory(
		softSpoken,
		_message,
		formattedMessageWithoutAnsi,
		_formattedMessageWithAnsi
	) {
		const configuredStdout = softSpoken.config.stdout;
		const lines = formattedMessageWithoutAnsi.split(os.EOL);

		let lastLineLength = lines[lines.length - 1].length;
		let l =
			softSpoken.getWidth() -
			lastLineLength -
			softSpoken.config.indentation.length;
		let i = l + 1;
		...
		return function (doneMessage, isCleared) {
			...
			if (++i > l || isCleared) {
				...
				if (l < 2 || (isCleared && isOnNewLine)) {
					...
					const indent = primitives.getLeftIndentationString(
						softSpoken.config.indentation,
						softSpoken.indentationLevel
					);
					configuredStdout.write(os.EOL);
			...
			configuredStdout.write(
				primitives.formatString('.', softSpoken.colors.spinnerSpinning)
			);
		};
	},

	spriteSpinner: function spriteSpinnerFactory(
		softSpoken,
		_message,
		formattedMessageWithoutAnsi,
		_formattedMessageWithAnsi
			...
			configuredStdout.write(
				(isOnNewLine ? '' : ' ') +
					primitives.formatString(
						...
						softSpoken.colors.spinnerSpinning
					)
			);
		};
	},
