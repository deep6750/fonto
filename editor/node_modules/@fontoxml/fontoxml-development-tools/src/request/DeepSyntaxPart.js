...
function getValueFromPath(nameParts, obj) {
	return nameParts.reduce(
		(o, part) => (o && o[part] ? o[part] : undefined),
		obj
	);
}
...
export default class DeepSyntaxPart {
	static [symbols.spliceInputFromParts](parts) {
		...
		if (parts[0] === '-') {
			...
			return [
				deepName,
				getValueFromPath(deepName.split('.'), this.cloneDefault()) ||
					true,
			];
		}
	...
	static [symbols.createContributionToRequestObject](
		propertyName,
		accumulated,
		value
	) {
		const accumulatedProperty =
			accumulated &&
			accumulated[propertyName] &&
			accumulated[propertyName][this.name];
		const contribution = {
			[propertyName]: {
				[this.name]: accumulatedProperty || this.cloneDefault() || {},
			},
		};
